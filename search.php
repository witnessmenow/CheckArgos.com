...
$infoFileName = "search.csv";
require_once('ListOfProductsDisplayFunctions.php');
require_once('TextSearchFunctions.php');
include_once('header.php');


echo '<font face="Arial" size="6">Text Search:</font>';
echo "<br />";
echo 'Enter your search, select a store and a price range to view a list of in stock items. (Limited to 100)';
...
echo '<form name="input" action="'.$_SERVER['PHP_SELF'].'" method="get">
			<table id="inputs" width="800">
				<tr>
					<td> Search Text: </td>
					<td><input name="searchText" value="'.$searchText.'" size="30" type="text"></td>
					
					<td> Select Store: </td>
					<td colspan="3"><select name="storeName">';
...
if ($searchText == "" || $searchText == "invalid")
{
...
{	
	...
	fputcsv ($infoFile , array(time(), $searchText, $storeName, $minPrice, $maxPrice, $sectionSelected), ',');
	fclose ($infoFile);
	
	$storeId = $stores[$storeName];
	
	$products = advancedSearchProducts($searchText, $sectionSelected, $sectionNumber[$sectionSelected], $minPrice, $maxPrice, "100", "1", $searchPreference);
